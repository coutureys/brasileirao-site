...
export const generalLimiter = rateLimit({
  ...
  legacyHeaders: false,
  // sem keyGenerator custom: o padrão já usa o IP de forma segura p/ IPv6
  handler: (req, res) => {
    res.status(429).json({
...
export const loginLimiter = rateLimit({
  ...
  // usa o e-mail como chave; cai pro IP (tratado p/ IPv6) se não houver
...
export const commentLimiter = rateLimit({
  ...
  // usa o clientId como chave; cai pro IP (tratado p/ IPv6) se não houver
  keyGenerator: (req) => req.body?.clientId || ipKeyGenerator(req.ip),
})
